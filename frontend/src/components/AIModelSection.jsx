...
function AIModelSection({ mode, inputSignal, outputSignal, onModelResult, onComparisonChange }) {
  ...
  const [stemGains, setStemGains] = useState({});
  const [isRemixing, setIsRemixing] = useState(false);
  const [playingStem, setPlayingStem] = useState(null);
  
  // Voice separation states (keep existing)
  const [separatedVoices, setSeparatedVoices] = useState(null);
  const [voiceGains, setVoiceGains] = useState({});
  const [playingVoice, setPlayingVoice] = useState(null);
  ...
  const processWithAI = async () => {
    ...
    try {
      if (mode === "musical") {
        ...
        const initialGains = {};
        Object.keys(result.stems).forEach((stemKey) => {
          initialGains[stemKey] = 1.0;
        });
        setStemGains(initialGains);

        // Initial mix with all stems at 1.0
        await remixStems(result.stems, initialGains);
  ...
  const remixStems = async (stems, gains) => {
    if (!stems) return;
    
    setIsRemixing(true);
    try {
      // Prepare stems with gains
      const stemsWithGains = {};
      Object.keys(stems).forEach((stemKey) => {
        stemsWithGains[stemKey] = {
          data: stems[stemKey].data,
          gain: gains[stemKey] !== undefined ? gains[stemKey] : 1.0,
        };
      });
      ...
      // Store and restore comparison mode
      const currentMode = comparisonMode;
      previousComparisonModeRef.current = currentMode;
      setComparisonMode(null);
      onComparisonChange(null);
  ...
  const remixVoices = async (voices, gains) => {
    // Keep existing voice remix code
    if (!voices) return;
  ...
  const handleStemGainChange = (stemKey, gain) => {
    const newGains = { ...stemGains, [stemKey]: gain };
    setStemGains(newGains);
    
    if (separatedStems) {
      remixStems(separatedStems, newGains);
    }
  };
    ...
    <section className="section ai-model-section">
      ...
      <div className="ai-controls">
        <div className="ai-status">
          <span className="status-indicator loaded">✓</span>
          <span className="status-text">
            {mode === "musical" ? "Demucs Model Ready" : mode === "human" ? "Voice Separation Model Ready" : "Mock Model Ready"}
          </span>
        </div>
      ...
      {/* Music stem separation controls */}
      {mode === "musical" && separatedStems && (
        <div className="voice-separation-controls">
          <h3 className="voice-separation-title">🎵 Separated Stems</h3>
          <div className="voice-sliders-container">
            {Object.keys(separatedStems).map((stemKey) => (
              <div key={stemKey} className="voice-slider-item">
                <label className="voice-slider-label">
                  {getStemDisplayName(stemKey)}
                </label>
                <div className="voice-slider-wrapper">
                  <input
                    type="range"
                    ...
                    max="2"
                    step="0.01"
                    value={stemGains[stemKey] !== undefined ? stemGains[stemKey] : 1.0}
                    onChange={(e) => handleStemGainChange(stemKey, parseFloat(e.target.value))}
                    className="voice-slider"
                  />
                  <span className="voice-slider-value">
                    {(stemGains[stemKey] !== undefined ? stemGains[stemKey] : 1.0).toFixed(2)}
                  </span>
                  <button
                    className="voice-play-btn"
                    onClick={() => playStem(stemKey)}
                    title={playingStem === stemKey ? "Stop" : "Play"}
                  >
                    {playingStem === stemKey ? "⏸️" : "▶️"}
                  </button>
                </div>
              </div>
            ))}
      ...
      {/* Voice separation controls (keep existing) */}
      {mode === "human" && separatedVoices && (
        <div className="voice-separation-controls">
          <h3 className="voice-separation-title">🎤 Separated Voices</h3>
          <div className="voice-sliders-container">
